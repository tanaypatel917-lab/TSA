...
export const MODES: Record<Difficulty, { label: string; seconds: number; lives: number; field: number; count: number; multiplier: number; blurb: string }> = {
  relaxed: { label: "Relaxed", seconds: RUN.seconds, lives: 3, field: 3, count: 8, multiplier: 1, blurb: "No clock and no time bonus." },
  standard: { label: "Standard", seconds: RUN.seconds, lives: 3, field: 3, count: 8, multiplier: 1, blurb: "75 seconds and three lives." },
  expert: { label: "Expert", seconds: 50, lives: 2, field: 4, count: 10, multiplier: 1.5, blurb: "50 seconds, two lives, four crates at once and ×1.5 points." }
};
...
function spawn(run: Run, spots: readonly Point[]): Run {
  ...
  while (field.length < run.fieldSize && next < run.order.length) {
...
export function startRun(mission: Mission, spots: readonly Point[], options: { seed: number; relaxed?: boolean; mode?: Difficulty }): Run {
  const mode = options.mode ?? (options.relaxed ? "relaxed" : "standard");
  const config = MODES[mode];
  const order = shuffled(mission.items.length, options.seed).slice(0, Math.min(config.count, mission.items.length));
  return spawn({ moduleId: mission.moduleId, order, next: 0, spawned: options.seed % spots.length, field: [], carrying: null, score: 0, combo: 0, mistakes: 0, results: [], timeLeft: config.seconds, relaxed: mode === "relaxed", mode, lives: config.lives, seconds: config.seconds, fieldSize: config.field, multiplier: config.multiplier, over: null }, spots);
}
...
export function deliver(run: Run, gate: string, mission: Mission): Delivery {
  ...
  const points = ok ? Math.round(RUN.points * combo * run.multiplier) : 0;
...
export function daySeed(day: string) {
  let hash = 2166136261;
  for (const character of day) hash = Math.imul(hash ^ character.charCodeAt(0), 16777619) >>> 0;
  return hash;
}

export function dailyPick(day: string, count: number) {
  return daySeed(day) % count;
}
