...
export function MagneticButton({ href, children }: { href: string; children: React.ReactNode }) {
  ...
  return <motion.a ref={ref} href={href} className="btn-pill group w-full sm:w-auto" style={{ x, y }} onMouseMove={move} onMouseLeave={() => { x.set(0); y.set(0); }} data-cursor="hover">{children}<span className="ml-3 inline-block transition-transform group-hover:translate-x-1">↗</span></motion.a>;
}
