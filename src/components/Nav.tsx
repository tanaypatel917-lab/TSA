...
export function Nav() {
  ...
  </div>{open && <div id="mobile-menu" ref={menuRef} role="dialog" aria-modal="true" aria-label="Mobile navigation" className="fixed inset-0 z-30 flex min-h-screen flex-col justify-center bg-ink px-5 text-paper md:hidden"><button className="absolute right-5 top-7 font-mono text-[10px] uppercase tracking-[.16em] text-paper/70" onClick={closeMenu}>Close ×</button><p className="eyebrow text-paper/50">Navigation</p><nav className="mt-8 space-y-5">{links.map(([label, href], index) => <Link key={href} href={href} onClick={closeMenu} className="block font-display text-5xl italic" style={{ animation: `reveal .5s ${index * 0.06}s both` }}>{label}<span className="ml-3 font-mono text-sm not-italic text-accent">0{index + 1}</span></Link>)}</nav><div className="mt-8"><SoundToggle /></div></div>}</header>;
}
