...
export function Nav() {
  ...
  useEffect(() => {
    ...
    const update = () => setStoryActive(root.dataset.storyActive === "1");
  ...
  return <motion.header onFocusCapture={() => setHidden(false)} animate={{ y: reduced || !hidden ? 0 : "-100%" }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }} className={`fixed inset-x-0 top-0 z-30 ${storyActive ? "text-paper" : "text-ink"} ${scrolled && !storyActive ? "is-scrolled border-b border-line bg-paper/85 backdrop-blur-md" : ""}`}><div className="shell flex h-20 items-center justify-between">
    ...
    <button ref={triggerRef} aria-label={open ? "Close menu" : "Open menu"} aria-controls="mobile-menu" aria-expanded={open} className="relative z-40 flex items-center gap-3 py-2 font-mono text-[10px] font-bold uppercase tracking-[.16em] md:hidden" onClick={() => open ? closeMenu() : setOpen(true)} data-cursor="hover"><span className="relative grid h-5 w-6 place-items-center"><span className={`absolute h-px w-6 bg-current transition-transform ${open ? "rotate-45" : "-translate-y-1.5"}`} /><span className={`absolute h-px w-6 bg-current transition-transform ${open ? "-rotate-45" : "translate-y-1.5"}`} /></span><span>Menu</span></button>
