...
export function Nav() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-30 transition-all duration-300 ${scrolled ? "is-scrolled border-b border-line bg-paper/85 backdrop-blur-md" : ""}`}><div className="shell flex h-20 items-center justify-between">
    ...
    <button aria-label={open ? "Close menu" : "Open menu"} aria-controls="mobile-menu" aria-expanded={open} className="relative z-40 flex items-center gap-3 py-2 font-mono text-[10px] font-bold uppercase tracking-[.16em] md:hidden" onClick={() => setOpen(!open)} data-cursor="hover"><span className="relative grid h-5 w-6 place-items-center"><span className={`absolute h-px w-6 bg-ink transition-transform ${open ? "rotate-45" : "-translate-y-1.5"}`} /><span className={`absolute h-px w-6 bg-ink transition-transform ${open ? "-rotate-45" : "translate-y-1.5"}`} /></span><span>Menu</span></button>
  </div>{open && <div id="mobile-menu" ref={menuRef} role="dialog" aria-modal="true" aria-label="Mobile navigation" className="fixed inset-0 z-30 flex min-h-screen flex-col justify-center bg-ink px-5 text-paper md:hidden"><button className="absolute right-5 top-7 font-mono text-[10px] uppercase tracking-[.16em] text-paper/70" onClick={() => setOpen(false)}>Close ×</button><p className="eyebrow text-paper/50">Navigation</p><nav className="mt-8 space-y-5">{links.map(([label, href], index) => <Link key={href} href={href} onClick={() => setOpen(false)} className="block font-display text-5xl italic" style={{ animation: `reveal .5s ${index * 0.06}s both` }}>{label}<span className="ml-3 font-mono text-sm not-italic text-accent">0{index + 1}</span></Link>)}</nav></div>}</header>;
}
