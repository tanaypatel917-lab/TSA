...
export function Capstone({ activity, moduleId }: { activity: Extract<Activity, { kind: "capstone" }>; moduleId: string }) {
  ...
  return <div className="card"><p className="eyebrow">Your project plan</p><h2 className="mt-2 text-2xl font-bold">{activity.title}</h2><p className="mt-2 text-slate-600">{activity.intro}</p><Stagger className="mt-5 space-y-5">{activity.steps.map((step, index) => <StaggerItem key={step.id}><label className="block font-semibold"><span>{index + 1}. {step.prompt}</span><textarea rows={3} value={answers[step.id] ?? ""} onChange={(event) => setAnswers({ ...answers, [step.id]: event.target.value })} className="mt-2 w-full rounded-xl border-slate-300 font-normal" placeholder={step.placeholder} /></label></StaggerItem>)}</Stagger><div className="mt-5 flex flex-wrap gap-3"><button className="button-primary" onClick={finish} disabled={!ready || completed}>{completed ? "Plan complete" : "Complete capstone"}</button><button className="button-secondary" onClick={download} disabled={!ready}>Download plan</button></div></div>;
}
