...
export default function AccountPage() {
  ...
  if (user) {
    ...
    return <div className="shell py-32"><div className="grid gap-8 lg:grid-cols-12"><div className="lg:col-span-8"><p className="eyebrow"><span className="mr-3 text-accent">01</span> Your account</p><SplitText as="h1" className="mt-7 font-display text-[clamp(3rem,8vw,7rem)] leading-[.88]">Keep your<br /><em>signal.</em></SplitText></div><p className="self-end text-lg leading-relaxed text-ink/65 lg:col-start-9 lg:col-span-4">Your account syncs only your learning progress. Practice content and answers stay out of the cloud.</p></div><Stagger className="mt-20 grid gap-6 sm:grid-cols-2"><StaggerItem className="card"><p className="eyebrow">Signed in as</p><p className="mt-5 break-all font-display text-3xl italic">{user.email}</p><span className="mt-6 inline-flex border border-line px-3 py-2 font-mono text-[10px] uppercase tracking-wider">{syncLabel(syncStatus, lastSyncedAt)}</span></StaggerItem><StaggerItem className="card"><p className="eyebrow">Local progress</p><p className="mt-5 font-mono text-5xl">{state.xp} <span className="text-sm text-ink/50">XP</span></p><p className="mt-2 font-display text-2xl italic">{level.name}</p></StaggerItem></Stagger>{error && <p className="mt-8 text-sm text-accent" role="alert">{error}</p>}{notice && <p className="mt-8 text-sm text-ink/70" role="status">{notice}</p>}<div className="mt-12 flex flex-wrap gap-5 border-t border-line pt-8"><button className="btn-pill" onClick={leaveAccount} disabled={busy}>Sign out</button><button className="btn-ghost" onClick={removeCloudProgress} disabled={busy}>Delete cloud data ↗</button></div><p className="mt-8 max-w-prose text-sm leading-relaxed text-ink/60">Local progress stays on this device even if you sign out or delete your cloud data.</p></div>;
  }
